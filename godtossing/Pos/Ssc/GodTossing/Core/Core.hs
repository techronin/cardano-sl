...
genCommitmentAndOpening t pks
    ...
    | otherwise  = do
        pks' <- NE.sort <$> traverse fromBinaryM pks
        liftIO . runSecureRandom $
            convertRes pks <$> genSharedSecret t pks'
  where
    n = fromIntegral (length pks)
    convertRes (toList -> ps) (secret, proof, shares) =
        ( Commitment
...
verifyCommitment Commitment {..} = fromMaybe False $ do
    -- The proof can be deserialized
    _ <- fromBinaryM commProof
