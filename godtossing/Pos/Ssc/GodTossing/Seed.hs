...
import           Control.Lens                 (each, traverseOf, _Left)
...
import           Pos.Crypto                   (DecShare, Secret, VssPublicKey,
                                               recoverSecret, verifySecret)
...
calculateSeed commitments' binVssKeys openings lShares richmen = do
    ...
    let recovered :: HashMap StakeholderId (Maybe Secret)
        ...
        recover key = do
            ...
            (_, Commitment{..}, _) <- HM.lookup key commitments
            -- Calculate the threshold from amount of shares:
            let threshold = fromIntegral $
                    vssThreshold $ sum (HM.map length commShares)
            -- Get shareholders and their amounts of shares:
            shareholders <-
                HM.toList commShares
                  & map (over _2 length)                -- calc share amounts
                  & traverseOf (each . _1) fromBinaryM  -- decode keys
                  & fmap (sortWith fst)                 -- sort by keys
            -- Recover the secret
            ...
            guard (verifySecret proof secret)
            pure secret
